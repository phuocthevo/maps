...
def group_by_centroid(restaurants, centroids):
    ...
    for x in restaurants:
        temp_element=list([find_closest(restaurant_location(x),centroids),x])
        centroid_restaurant.append(temp_element)
    return group_by_first(centroid_restaurant)


def find_centroid(cluster):
    """Return the centroid of the locations of the restaurants in cluster."""
    latitude_list = [restaurant_location(x)[0] for x in cluster]
    longitude_list = [restaurant_location(x)[1] for x in cluster]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictor_list = [list(find_predictor(user, reviewed, x)) for x in feature_fns]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for x in restaurants:
        if x in reviewed:
            reviews_dict[restaurant_name(x)]=user_rating(user,restaurant_name(x))
        else:
            reviews_dict[restaurant_name(x)]=predictor(x)
    return reviews_dict
